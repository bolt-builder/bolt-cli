...
import { LocationServiceMap, locationServiceMapLayer } from "@bolt-ai/core/location-services"
import { Ripgrep } from "@bolt-ai/core/ripgrep"
import { FSUtil } from "@bolt-ai/core/fs-util"
import { Location } from "@bolt-ai/core/location"
import { AbsolutePath, RelativePath } from "@bolt-ai/core/schema"
...
export const fileHandlers = HttpApiBuilder.group(InstanceHttpApi, "file", (handlers) =>
  Effect.gen(function* () {
    ...
    const list = Effect.fn("FileHttpApi.list")(function* (ctx: { query: { path: string } }) {
      ...
      if (!FSUtil.contains(directory, requested)) return yield* Effect.die(new Error("Path escapes the location"))
