...
const bolt = path.resolve(dir, "../../opencode")

await $`bun dev generate > ${dir}/openapi.json`.cwd(bolt)
...
// Patch a @hey-api/openapi-ts codegen bug (fixed upstream in 0.99): SseFn
// incorrectly passed the endpoint's TError into the second generic of
// ServerSentEventsResult, which is the AsyncGenerator's TReturn slot.
// Iterator return values have nothing to do with HTTP errors, and any
// consumer that calls `.return()` or returns from a mock generator gets
// type-checked against the wrong shape. Drop the arg so TReturn defaults to
// void when the buggy output is present; newer versions already emit the
// fixed signature.
...
if (!sseTypesPatched.includes("=> Promise<ServerSentEventsResult<TData>>")) {
  throw new Error(`SseFn signature not found; @hey-api/openapi-ts output may have changed (${sseTypesPath})`)
}
