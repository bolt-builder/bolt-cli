...
- **Via Mise**
  ...
  ```bash
  mise use -g github:bolt-builder/bolt-cli
  ```

- **Via Docker**

  ```bash
  docker run -it --rm ghcr.io/bolt-builder/bolt-cli
  ```

Le support de l'installation de OpenCode sur Windows à l'aide de Bun est actuellement en cours de développement.

Vous pouvez également récupérer le binaire dans les [Releases](https://github.com/Bolt-builder/bolt-cli/actions/workflows/publish.ym/releases).
