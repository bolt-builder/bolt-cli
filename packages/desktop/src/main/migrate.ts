...
// The legacy `opencode.settings.dat` is special: it maps to the settings store
// (SETTINGS_STORE). All other .dat files keep their full filename as the
// electron-store name so they match what the renderer already passes via IPC
// (e.g. `"default.dat"`, `"opencode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // The legacy opencode.settings.dat → the electron settings store (SETTINGS_STORE).
  ...
  const storeName = filename === "opencode.settings.dat" ? SETTINGS_STORE : filename
