...
describe("memory core package", () => {
  ...
  test("prepare removes legacy decisions once from owned memory roots", async () => {
    await use(async (t) => {
      ...
      await BoltMemory.status({ root: t.root })
      ...
      await BoltMemory.status({ root: other })
  ...
  test("prepare ignores legacy decisions cleanup failures", async () => {
    ...
    try {
      await use(async (t) => {
        ...
        const status = await BoltMemory.status({ root: t.root })
  ...
  test("prepare ignores corrupt manifests during legacy cleanup", async () => {
    ...
    try {
      await use(async (t) => {
        ...
        await writeFile(paths.manifest, '{"kind":"bolt-memory","version":1}\n')
  ...
  test("index suppresses bulk session digests covered by typed memory", async () => {
    await use(async (t) => {
      ...
      // ...but a compact pointer keeps its id targetable via memory_recall mode=digest.
