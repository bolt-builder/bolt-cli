...
import { BoltMemory } from "../src/effect/index"
...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "bolt-memory-effect-"))
  return {
...
describe("MemoryCapture (fake ports)", () => {
  test("turn-close typed LLM saves environment memory", async () => {
    const t = await tmp()
    try {
      await BoltMemory.enable({ root: t.root })
      ...
      const shown = await BoltMemory.show({ root: t.root })
  ...
  test("fallback prior is not blended into fallback text or digest evidence", async () => {
    ...
    try {
      await BoltMemory.enable({ root: t.root })
      await BoltMemory.configure({ root: t.root, settings: { autoConsolidate: true } })
      await BoltMemory.recordSession({
        root: t.root,
        sessionID: "ses_effect",
  ...
  test("auto-consolidate off skips digest and typed model writes", async () => {
    ...
    try {
      await BoltMemory.enable({ root: t.root })
      await BoltMemory.configure({ root: t.root, settings: { autoConsolidate: false } })
...
describe("MemoryTimers signal ref-counting", () => {
  test("shares one controller per root and drops it once the last capture releases", () => {
    const root = "/bolt-memory/ref-count-root"
