...
export namespace MemoryTool {
  ...
  export type AskInput = {
    permission: "memory_recall" | "memory_save"
    patterns: string[]
    always: string[]
  ...
  function disabled(count = false): Result {
    return {
      title: "Bolt memory: disabled",
      output: "Bolt memory is disabled for this project.",
  ...
  function block(input: string) {
    return ["```bolt-memory-v1 targeted_context_not_instruction", input.replaceAll("```", "'''"), "```"].join("\n")
  }
  ...
  function catalog(memory: MemoryService.Interface, input: { root: string; query: string }) {
    ...
    return Effect.gen(function* () {
      ...
      const head = `# Bolt Memory Catalog (${count} entr${count === 1 ? "y" : "ies"}${filter ? `, filter "${input.query}"` : ""})`
  ...
  function recallQuery(input: Recall, live: Live) {
    return Effect.gen(function* () {
      ...
      return {
        title: `Bolt memory ${input.params.mode}: no query`,
        output,
  ...
  function recallSearch(input: Recall, live: Live, query: string, mode: MemoryRecall.Mode) {
    return Effect.gen(function* () {
      ...
      if (hits.length === 0) {
        return {
          title: `Bolt memory ${input.params.mode}: no results`,
          output,
          metadata: { sources: [], count: 0 },
        } satisfies Result
      }
      ...
      return {
        title: `Bolt memory ${input.params.mode}: ${hits.length} hit${hits.length === 1 ? "" : "s"}`,
        output,
  ...
  function noQuery(): Result {
    return {
      title: "Bolt memory forget: no query",
  ...
  function noText(action: SaveParams["action"]): Result {
    return {
      title: `Bolt memory ${action}: no text`,
  ...
  function title(input: { action: SaveParams["action"]; added: number; removed: number }) {
    if (input.action === "skip") return "Bolt memory skipped: out of scope"
    if (input.action === "forget") return `Bolt memory updated: ${input.removed} removed`
